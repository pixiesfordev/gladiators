...
#region  Types
		/// <summary> Whether odd columns are shifted up or down. </summary>
...
#region  Compute
		protected override void ComputeLines(LineSegment[][] lineSets) {
			...
			System.Action<LineSegment[], Vector3> zigZagLines = (lines, o) => {
				...
				for (var column = Left; column <= Right; ++column) {
					...
					for (var row = Bottom; row <= Top; ++row) {
						...
						for (var layer = Back; layer <= Front; ++layer) {
							if (column != Right) {
								lines[i++] = new LineSegment(l, l + right[u]);
							}
							if (column == Left) { // Leftmost zig-zag lines
								var extra = from.x - Left;
								lines[i++] = new LineSegment(l, l + extra * right[u == 0 ? 1 : 0]);
							}
							if (column == Right) { // Rightmost zig-zag lines
								var extra = to.x - Right;
								lines[i++] = new LineSegment(l, l + extra * right[u]);
							}
							l += forward;
						}
						r += up;
			...
			System.Action<LineSegment[], Vector3> straightLines = (lines, o) => {
				var i = 0;  // Iterator through the lines array.
				...
				for (var column = Left; column <= Right; ++column) {
					...
					for (var layer = Back; layer <= Front; ++layer) {
						var v = l + extra * up;
						lines[i++] = new LineSegment(v, v + length * up);
						l += forward;
					}
					o += right[u];
				}
			...
			System.Action<LineSegment[], Vector3> layerLines = (lines, o) => {
				var i = 0;  // Iterator through the lines array.
				...
				for (var column = Left; column <= Right; ++column) {
					...
					var r = o;
					for (var row = Bottom; row <= Top; ++row) {
						var v = r + extra * forward;
						lines[i++] = new LineSegment(v, v + length * forward);
						r += up;
					}
					o += right[u];
				}
			...
			zigZagLines(lineSets[0], origin);
			straightLines(lineSets[1], origin);
			layerLines(lineSets[2], origin);

...
#region  Types
		...
		public enum OddColumnShift {
			/// <summary> Every odd column is shifted upwards. </summary>
			Up,
			/// <summary> Every odd column is shifted downwards. </summary>
			Down,
			/// <summary> Every odd column is clipped. </summary>
			Clipped
		}
#endregion  // Types
...
#region  Count
		protected override (int x, int y, int z) CountLines() {
			...
			var xCount = layers * (1 * hexesH * hexesV + 1 * hexesH);
			...
			// NE
			var yCount = layers * (2 * hexesH * hexesV + 2 * hexesV + 1 * hexesH - 1);
			...
			// per row
			var zCount = 2 * hexesH * hexesV + 1 * hexesH + 1 * hexesV + 1 * hexesH + 1 * hexesV;
			...
			if (Shift == OddColumnShift.Clipped) {
				...
				xCount -= (1 * oddColumns                                                   ) * layers;
				yCount -= (2 * oddColumns + (IsEven(left) ? 0 : 1) + (IsEven(right) ? 0 : 1)) * layers;
				zCount -=  2 * oddColumns + (IsEven(left) ? 0 : 1) + (IsEven(right) ? 0 : 1)          ;
			}
	
			if (FlatSides) {
				Swap<int>(ref xCount, ref yCount);
			}
...
#region  Compute
		protected override void ComputeLines(LineSegment[][] lineSegments) {
			...
			if (!pointed) {
				...
				Swap<LineSegment[]>(ref lineSegments[0], ref lineSegments[1]);
			}
			...
			System.Action<Vector3, int, int, int> drawHex = (hex, c, r, l) => {
				...
				System.Action<Vector3, Vector3> horizontalLine =
					(v1, v2) => ContributeLine(lineSegments[0], hex, v1, v2, ref iterator_x);
				System.Action<Vector3, Vector3> verticalLine =
					(v1, v2) => ContributeLine(lineSegments[1], hex, v1, v2, ref iterator_y);
			...
			System.Action<Vector3, int, int> drawLayer = (hex, column, row) => {
				...
				System.Action<Vector3> layerLine =
					vertex => ContributeLine(lineSegments[2], hex, vertex, back, front, ref iterator_z);
			...
			var hexColumn = DetermineOrigin(pointed, downShift, leftEdge, bottomEdge, Back);

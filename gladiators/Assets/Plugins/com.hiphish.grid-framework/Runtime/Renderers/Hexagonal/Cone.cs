...
#region  Explanation comment
			...
			if (RadiusFrom == 0) {
				...
				for (var i = aMin; i < aMax; ++i){
					...
					IncrementCountXY(i + 1, rMin    , ref xCount, ref yCount);
					IncrementCountXY(i + 2, rMin - 1, ref xCount, ref yCount);
					zCount += 2 * rMin - 1; // 2 * for inner, + 1 radial
				}
				// add the closing line for the last radial hex
				if ((aMax - aMin) % 6 != 0 || aMax == aMin) {
					IncrementCountXY(aMax + 1, 1, ref xCount, ref yCount);
					zCount += 2;
				}
			}
			...
			for (var i = rMin; i <= rMax; ++i) {
				for (var j = aMin; j < aMax; ++j) {
					...
					for (var k = 0; k < i; ++k) {
						if (k == 0) {
							IncrementCountXY(j + 2, 1, ref xCount, ref yCount);
							++zCount;
						}
						// now the inner hexes
						IncrementCountXY(j + 0, 1, ref xCount, ref yCount);
						IncrementCountXY(j + 1, 1, ref xCount, ref yCount);
						IncrementCountXY(j + 2, 1, ref xCount, ref yCount);
				...
				if ((aMax - aMin) % 6 != 0 || aMax == aMin) {
					...
					if ((aMax - aMin) % 6 != 5 || i > 1) {
						IncrementCountXY(aMax+0, 1, ref xCount, ref yCount);
					}

					IncrementCountXY(aMax+0, 1, ref xCount, ref yCount);
					IncrementCountXY(aMax+1, 1, ref xCount, ref yCount);
					IncrementCountXY(aMax+2, 2, ref xCount, ref yCount);
			...
			Swap<int>(ref xCount, ref yCount, flat);
...
#region  Compute
		protected override void ComputeLines(LineSegment[][] lineSets) {
			...
			var origin = Grid.HerringUpToWorld(new Vector3(OriginX, OriginY, 0));
			...
			if (radius[0] == 0) {
				for (var i = 0; i < 6; ++i) {
					ContributeLayer(origin, 0, i, forward, ref iterator_z, lineSets);
				}
				for (var i = back; i <= front; ++i) {
					ContributeCenter(currentHex, ref iterator_x, ref iterator_y, lineSets);
					currentHex += forward;
				}
				...
				for (var i = angle[0]; i < angle[1]; ++i) {
					...
					for (var j = 0; j < radius[0]; ++j) {
						...
						for (var k = back; k <= front; ++k) {
							...
							if (j != 0) {
								ContributeConnecting(currentHex, i%6, ref iterator_x, ref iterator_y, lineSets);
							}
							ContributeInner(currentHex, i%6, ref iterator_x, ref iterator_y, lineSets);
							currentHex += forward;
						}
						currentHex -= (front+1) * forward; // back to the origin layer
						if (j != 0) {
							ContributeLayer(currentHex, i%6, 5, forward, ref iterator_z, lineSets);
						}
						ContributeLayer(currentHex, i%6, 4, forward, ref iterator_z, lineSets);
				...
				if ((angle[1] - angle[0]) % 6 != 0 || angle[1] == angle[0]) {
					...
					for (var j = back; j <= front; ++j) {
						ContributeInner(currentHex, angle[1]%6, ref iterator_x, ref iterator_y, lineSets);
						currentHex += forward;
					}
					currentHex -= (front+1) * forward;
					ContributeLayer(currentHex, angle[1]%6, 3, forward, ref iterator_z, lineSets);
					ContributeLayer(currentHex, angle[1]%6, 4, forward, ref iterator_z, lineSets);
				}
			}
			...
			for (var i = radius[0]; i <= radius[1]; ++i) {
				for (var j = angle[0]; j < angle[1]; ++j) {
					...
					for (var k = 0; k < i; ++k) {
						for (var l = back; l <= front; ++l) {
							if (k == 0) {
								ContributeConnecting(currentHex, j%6, ref iterator_x, ref iterator_y, lineSets);
							}
							ContributeBottom(currentHex, j%6, ref iterator_x, ref iterator_y, lineSets);
							ContributeSides(currentHex, j%6, ref iterator_x, ref iterator_y, lineSets);
							currentHex += forward;
						}
						// now the inner hexes
						currentHex -= (front+1) * forward; // back to origin layer
						if (k == 0) {
							ContributeLayer(currentHex, j%6, 5, forward, ref iterator_z, lineSets);
						}
						ContributeLayer(currentHex, j%6, 0, forward, ref iterator_z, lineSets);
						ContributeLayer(currentHex, j%6, 1, forward, ref iterator_z, lineSets);
						currentHex += back * forward; // back forward
				...
				if ((angle[1] - angle[0]) % 6 != 0 || angle[1] == angle[0]) {
					...
					currentHex += back * forward;
					for (var j = back; j <= front; ++j) {
						ContributeConnecting(currentHex, angle[1]%6, ref iterator_x, ref iterator_y, lineSets);
						ContributeBottom(currentHex, angle[1]%6, ref iterator_x, ref iterator_y, lineSets);
						ContributeSides(currentHex, angle[1]%6, ref iterator_x, ref iterator_y, lineSets);
						// If we don't do this a line will be drawn twice
						if ((angle[1] - angle[0]) % 6 != 5 || i > 1) {
							ContributeClosing(currentHex, angle[1]%6, ref iterator_x, ref iterator_y, lineSets);
						}
						currentHex += forward;
					}
					currentHex -= (front+1) * forward; // back to origin layer
					ContributeLayer(currentHex, angle[1]%6, 0, forward, ref iterator_z, lineSets);
					ContributeLayer(currentHex, angle[1]%6, 1, forward, ref iterator_z, lineSets);
					ContributeLayer(currentHex, angle[1]%6, 2, forward, ref iterator_z, lineSets);
					ContributeLayer(currentHex, angle[1]%6, 5, forward, ref iterator_z, lineSets);
				}
			}
		}
...
#region  Contribute
		...
		private int ContributeEdge(int index, Vector3 hex, int i, int iterator, LineSegment[][] lineSets) {
			var from = hex + vertices[(i + 0) % 6];
			var   to = hex + vertices[(i + 1) % 6];
			lineSets[index][iterator] = new LineSegment(from, to);
			return ++iterator;
		}

		private void ContributeCenter(Vector3 hex, ref int iterator_x, ref int iterator_y, LineSegment[][] lineSets) {
			var pointed = Grid.Sides == GridFramework.Grids.HexGrid.Orientation.Pointed;
			if (pointed) {
				iterator_y = ContributeEdge(1, hex, 0, iterator_y, lineSets);
				iterator_x = ContributeEdge(0, hex, 1, iterator_x, lineSets);
				iterator_y = ContributeEdge(1, hex, 2, iterator_y, lineSets);
				iterator_y = ContributeEdge(1, hex, 3, iterator_y, lineSets);
				iterator_x = ContributeEdge(0, hex, 4, iterator_x, lineSets);
				iterator_y = ContributeEdge(1, hex, 5, iterator_y, lineSets);
			} else {
				iterator_y = ContributeEdge(1, hex, 0, iterator_y, lineSets);
				iterator_x = ContributeEdge(0, hex, 1, iterator_x, lineSets);
				iterator_x = ContributeEdge(0, hex, 2, iterator_x, lineSets);
				iterator_y = ContributeEdge(1, hex, 3, iterator_y, lineSets);
				iterator_x = ContributeEdge(0, hex, 4, iterator_x, lineSets);
				iterator_x = ContributeEdge(0, hex, 5, iterator_x, lineSets);
			}
		}
	
		private void ContributeConnecting(Vector3 hex, int a, ref int iterator_x, ref int iterator_y, LineSegment[][] lineSets) {
			int ix;
			var pointed = Grid.Sides == GridFramework.Grids.HexGrid.Orientation.Pointed;
			if (pointed) {
				ix = (a == 0 || a == 3) ? 0 : 1;
			...
			if (ix == 0) {
				iterator_x = ContributeEdge(ix, hex, a + 4, iterator_x, lineSets);
			} else {
				iterator_y = ContributeEdge(ix, hex, a + 4, iterator_y, lineSets);
			}
		}

		private void ContributeBottom(Vector3 hex, int a, ref int iterator_x, ref int iterator_y, LineSegment[][] lineSets) {
			int ix;
			var pointed = Grid.Sides == GridFramework.Grids.HexGrid.Orientation.Pointed;
			if (pointed) {
				ix = (a == 2 || a == 5) ? 0 : 1;
			...
			if (ix == 0) {
				iterator_x = ContributeEdge(ix, hex, a + 5, iterator_x, lineSets);
			} else {
				iterator_y = ContributeEdge(ix, hex, a + 5, iterator_y, lineSets);
			}
		}

		private void ContributeSides(Vector3 hex, int a, ref int iterator_x, ref int iterator_y, LineSegment[][] lineSets) {
			int ix1, ix2;
			var pointed = Grid.Sides == GridFramework.Grids.HexGrid.Orientation.Pointed;
			if (pointed) {
				ix1 = (a == 1 || a == 4) ? 0 : 1;
			...
			if (ix1 == 0) {
				iterator_x = ContributeEdge(ix1, hex, a + 0, iterator_x, lineSets);
			} else {
				iterator_y = ContributeEdge(ix1, hex, a + 0, iterator_y, lineSets);
			}
			if (ix2 == 0) {
				iterator_x = ContributeEdge(ix2, hex, a + 1, iterator_x, lineSets);
			} else {
				iterator_y = ContributeEdge(ix2, hex, a + 1, iterator_y, lineSets);
			}
		}

		private void ContributeClosing(Vector3 hex, int a, ref int iterator_x, ref int iterator_y, LineSegment[][] lineSets) {
			int ix;
			var pointed = Grid.Sides == GridFramework.Grids.HexGrid.Orientation.Pointed;
			if (pointed) {
				ix = (a == 2 || a == 5) ? 0 : 1;
			...
			if (ix == 0) {
				iterator_x = ContributeEdge(ix, hex, a + 2, iterator_x, lineSets);
			} else {
				iterator_y = ContributeEdge(ix, hex, a + 2, iterator_y, lineSets);
			}
		}

		private void ContributeInner(Vector3 hex, int a, ref int iterator_x, ref int iterator_y, LineSegment[][] lineSets) {
			int ix;
			var pointed = Grid.Sides == GridFramework.Grids.HexGrid.Orientation.Pointed;
			if (pointed) {
				ix = (a == 1 || a == 4) ? 0 : 1;
			...
			if (ix == 0) {
				iterator_x = ContributeEdge(ix, hex, a + 3, iterator_x, lineSets);
			} else {
				iterator_y = ContributeEdge(ix, hex, a + 3, iterator_y, lineSets);
			}
		}

		private void ContributeLayer(Vector3 hex, int a, int indx, Vector3 forward, ref int iterator_z, LineSegment[][] lineSets) {
			var from = hex + vertices[(indx+a)%6] + LayerFrom * forward;
			var   to = hex + vertices[(indx+a)%6] + LayerTo   * forward;
			lineSets[2][iterator_z] = new LineSegment(from, to);
			++iterator_z;
		}
#endregion  // Contribute

#region  Helpers
		private void IncrementCountXY(int i, int value, ref int xCount, ref int yCount) {
			i %= 6;
			switch (i) {
				case 0:
				case 1:

...
#region  Count
		protected override (int x, int y, int z) CountLines() {
			...
			var xCount = segmentCount * arcCount * layerCount; // total number of segments
			var yCount = sectorCount * layerCount; // total number of radial lines
			var zCount = arcCount * sectorCount; // total number of cylindrical lines
...
#region  Compute
		protected override void ComputeLines(LineSegment[][] lineSets) {
			...
			SegmentLines(  lineSets[0], origin);
			RadialLines(   lineSets[1], origin);
			CylindricalLines(lineSets[2], origin);
		...
		private void SegmentLines(LineSegment[] segments, Vector3 origin) {
			...
			for (var i = 0; i < layerCount; ++i) {  // for every layer
				...
				for (var j = 0; j < arcCount; ++j) {  // for every circle/arc
					...
					from = ContributePoint(r, _from[1] * angle, z, origin);

					for (var k = 1; k < segmentCount; ++k) { // for every segment
						to = ContributePoint(r, phi, z, origin);
						var pointIndex = i * arcCount * segmentCount + j * segmentCount + k - 1;

						segments[pointIndex++] = new LineSegment(from, to);
						from = to;
					...
					to = ContributePoint(r, _to[1] * angle, z, origin);
					segments[index] = new LineSegment(from, to);
		...
		private void RadialLines(LineSegment[] segments, Vector3 origin) {
			...
			for (var i = 0; i < layerCount; ++i) {
				...
				for (var j = 0; j < sectorCount; ++j) {
					...
					var  from = ContributePoint(_from[0], phi, z, origin);
					var    to = ContributePoint(  _to[0], phi, z, origin);
		...
		private void CylindricalLines(LineSegment[] segments, Vector3 origin) {
			...
			for (var i = 0; i < arcCount; ++i) {
				...
				for (var j = 0; j < sectorCount; ++j) {
					var index = i * sectorCount + j;
					var  from = ContributePoint(r, phi, fromZ, origin);
					var    to = ContributePoint(r, phi,   toZ, origin);

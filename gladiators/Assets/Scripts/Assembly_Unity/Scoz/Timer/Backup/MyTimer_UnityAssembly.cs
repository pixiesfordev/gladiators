...
using UnityEngine;
namespace Scoz.Func {
    public class MyTimer_UnityAssembly {
        ...
        public MyTimer_UnityAssembly(float _circleTime, Action _timeOutCB, bool _startRunTimer, bool _loop) {
            CurLoopTimes = 0;
            ...
            Loop = _loop;
            if (CircleTime == 0)
                WriteLog_UnityAssembly.LogWarning(string.Format("{0}'s CircleTime of MyTimer is 0", _timeOutCB.Method.Name));
        }
        public MyTimer_UnityAssembly(float _circleTime, Action _timeOutCB, Action _runTimeFunc, bool _startRunTimer, bool _loop) {
            CurLoopTimes = 0;
            CircleTime = _circleTime;
            CurTimer = CircleTime;
            TimeOutCB = _timeOutCB;

...
using UnityEngine;
namespace Scoz.Func {
    public class MyTimerT_UnityAssembly<T> {
        ...
        public MyTimerT_UnityAssembly(float _circleTime, Action<T> _timeOutCBWithT, Action<T> _runTimeFuncWithT, bool _startRunTimer, bool _loop) {
            CurLoopTimes = 0;
            ...
            Loop = _loop;
            if (CircleTime == 0)
                WriteLog_UnityAssembly.LogWarning(string.Format("{0}'s CircleTime of MyTimer is 0", TimeOutCBWithT.Method.Name));
        }
        /// <summary>
        /// 重置LoopTimes並重新倒數
        /// </summary>

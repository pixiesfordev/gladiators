...
public class WebViewManager : MonoSingletonA<WebViewManager> {
    ...
    public void ShowWebview(string url, bool setZoomEnabled = false, bool setLoadWithOverviewMode = true, bool setShowSpinnerWhileLoading = false, bool setBackButtonEnabled = false) {
        Rect rect = new Rect(0, 0, Screen.width, Screen.height);
        if (!string.IsNullOrEmpty(url)) {
            ShowWebview(url, rect);
    ...
    public void ShowWebview(string _url, Rect rect, RectTransform parentRectTransform) {
        ...
        if (CurrentUniWebView == null) {
            ...
            if (!string.IsNullOrEmpty(_url)) {
                ShowWebview(_url, rect, false, false, parentRectTransform);
